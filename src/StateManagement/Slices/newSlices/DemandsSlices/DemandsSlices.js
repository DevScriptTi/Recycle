...
const DemandsSlices = createSlice({
  name: "DemandsSlices",
  initialState: {
    data: [],
    links: [],
  ...
  reducers: {
    ...
    deleteDemand: (state, action) => {
      state.data = state.data.filter((item) => item.id !== action.payload);
    },
    updateDemand: (state, action) => {
...
export const {setDemands,addDemand,deleteDemand,updateDemand} = DemandsSlices.actions;
export default DemandsSlices.reducer;
